...
export type QuickOpenItem =
  | {id: string; kind: 'terminal'; label: string; detail: string; tab: TerminalTab}
  | {id: string; kind: 'session'; label: string; detail: string; session: SessionHost}
...
export function getQuickOpenItems(sessions: SessionHost[], terminals: TerminalTab[]): QuickOpenItem[] {
  return [
    ...terminals.map((tab) => ({id: `terminal:${tab.id}`, kind: 'terminal' as const, label: tab.title, detail: tab.cwd, tab})),
    ...sessions.map((session) => ({id: `session:${session.id}`, kind: 'session' as const, label: session.name, detail: `${session.username}@${session.host}`, session})),
  ]
}

export function activateQuickOpenItem(item: QuickOpenItem) {
  if (item.kind === 'terminal') terminalState.activeTerminalId = item.tab.id
