...
import {cancelTask, restoreBackgroundTasks, retryTask} from '../manageTask'
...
describe('manageTask', () => {
  ...
  it('restores background tasks from backend snapshots', async () => {
    setIpcMock(async (command) => {
      if (command === 'list_background_tasks') {
        return [
          {taskId: 'download-task', kind: 'sftp.download', title: 'Download file', detail: '/srv/app.log', status: 'running', progress: {transferredBytes: 25, totalBytes: 100}, error: null, updatedAtMs: 2},
          {taskId: 'failed-task', kind: 'sftp.upload', title: 'Upload file', detail: '/srv/app.env', status: 'failed', progress: {transferredBytes: 0, totalBytes: null}, error: 'network lost', traceId: 'task:failed-task', updatedAtMs: 1},
        ]
      }
      return undefined
    })

    await expect(restoreBackgroundTasks()).resolves.toBe(2)
    ...
    expect(taskItems.find((task) => task.id === 'failed-task')).toMatchObject({title: 'Upload file', detail: '/srv/app.env', error: 'network lost', progress: 0, status: 'failed', traceId: 'task:failed-task'})
  })

  it('cancels running background tasks through typed IPC', async () => {
    addTask({id: 'task', title: 'Download file', detail: '/tmp/file', progress: 50, status: 'running', traceId: 'task:task'})
    let payload: unknown = null
    setIpcMock(async (command, args) => {
      if (command === 'cancel_background_task') payload = args
      return undefined
    })
  ...
  it('reports cancel failures', async () => {
    addTask({id: 'task', title: 'Download file', detail: '/tmp/file', progress: 50, status: 'running', traceId: 'task:task'})
    setIpcMock(async (command) => {
      if (command === 'cancel_background_task') throw new Error('cancel failed')
      return undefined
    })

    await expect(cancelTask(taskItems[0])).rejects.toThrow('cancel_background_task failed: cancel failed')
    expect(feedbackState.toasts.at(-1)).toMatchObject({level: 'error', title: 'Failed to cancel Download file'})
    expect(feedbackState.toasts.at(-1)?.detail).toContain('Trace ID: task:task')
  })
