...
import {FolderTree, ListTodo, Search, Server} from '@lucide/vue'
...
export function registerDefaultSidebarPanels() {
  const disposables = [
    ...
    registerSidebarPanel({id: 'search', title: 'Search', icon: markRaw(Search), order: 40, commandId: 'workspace.openSearchPanel', component: markRaw(SearchPanel)}),
  ]
